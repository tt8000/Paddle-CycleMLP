...
from modules import DropPath, to_2tuple, Identity
...
class CycleFC(nn.Layer):
    ...
    def __init__(
        ...
        self.weight = self.create_parameter(shape=(out_channels, in_channels // groups, 1, 1), 
    ...
    def _calculate_fan_in_and_fan_out(self, tensor):
        ...
        num_output_fmaps = tensor.shape[2]
    ...
    def gen_offset(self):
        ...
        for i in range(offset.shape[1]//2):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        n_offset_grps = self.offset.shape[1] // (2 * weights_h * weights_w)
        n_weight_grps = input.shape[1] // self.weight.shape[1]
        return deform_conv2d(input, self.offset.expand((B, -1, H, W)), self.weight, self.bias, stride=self.stride,
                                padding=self.padding, dilation=self.dilation, deformable_groups=n_offset_grps,
                                groups=n_weight_grps)
...
class CycleMLP(nn.Layer):
    ...
    def forward(self, x):
        ...
        a = F.softmax(self.reweight(a).reshape((B, C, 3)).transpose((2, 0, 1)), axis=0).unsqueeze(2).unsqueeze(2)
...
class CycleNet(nn.Layer):
    ...
    def cls_init_weights(self, m):
        if isinstance(m, nn.Linear):
            ...
            m.weight_attr = nn.initializer.TruncatedNormal(std=.02)
            ...
            m.weight_attr = nn.initializer.Constant(1.0)
